...
llm = ChatOpenAI(model='gpt-4o', streaming=True, callbacks=[MyCallback()])

chain = requirements_prompt | llm | subrequirements_prompt | llm | code_prompt | llm

code = chain.invoke({ 'prompt': 'Create an image gallery that I can upload images to and then view.' }).content
